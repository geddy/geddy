...
module.exports = new (function () {
  ...
  this.loadAdapterForModel = function (modelItem, cb) {
    ...
    adapterObject = modelItem.adapter;
    // Is there a specific adapter defined for this model item?
    if (adapterObject) {
      adapterName = adapterObject.name;
  ...
  this.init = function (app, callback) {
    ...
    if((geddy.config.includePaths) && (geddy.config.includePaths.models)) {
    	for(var i=0; i < geddy.config.includePaths.models.length; i++) {
		  models = models.concat(utils._getConstructorsFromDirectory(path.join(geddy.config.includePaths.models[i])));
		}
	}
