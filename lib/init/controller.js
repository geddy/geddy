...
module.exports = new (function () {
  ...
  this.init = function (app, callback) {
	...
	if((geddy.config.includePaths) && (geddy.config.includePaths.controllers)) {
	  for(var j=0; j<geddy.config.includePaths.controllers.length; j++) {
	  	dirList = dirList.concat(_getDirList(geddy.config.includePaths.controllers[j]));
	  }
	}
