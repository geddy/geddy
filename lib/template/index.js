...
Templater.prototype.render = function (data, config) {
  ...
  nails.viewHelpers.registerData(data);
  Adapter.registerHelpers(nails.viewHelpers);
...
// Return the template object for a template from nails's template registry
getTemplateData = function (templateRoot, partialURL, parentNode, isLayout) {
  ...
  while (--i >= 0) {
    ...
    if (nails.templateRegistry[key] && nails.templateRegistry[key].registered) {
      templateData = nails.templateRegistry[key];
      break;
    }
  }
  ...
  if(!templateData) {
    ...
    if (isLayout) {
      ...
      if (templateData in nails.templateRegistry) {
        templateData = nails.templateRegistry[templateData];
      }
      // If default layout doesn't exist we have no other layout so throw error
      else {
        err = new nails.errors.InternalServerError('Layout template "' +
              partialURL + '" not found in ' + nails.utils.array.humanize(dirs));
        throw err;
      }
    }
    // If it's a normal template then it doesn't exist
    else {
      err = new nails.errors.InternalServerError('Partial template "' +
            partialURL + '" not found in ' + nails.utils.array.humanize(dirs));
      throw err;
    }
  }
