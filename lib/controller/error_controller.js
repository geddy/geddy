...
var fs = require('fs')
  ...
  , ErrorController
  , preferredFormatOrder;

// Views exist, render HTML errors as the default
if (fs.existsSync('app/views/errors')) {
  preferredFormatOrder = ['html', 'json', 'xml', 'txt'];
}
// No views, default to plaintext
else {
  preferredFormatOrder = ['txt', 'json', 'xml'];
}
...
ErrorController = function (req, resp) {
  ...
  this.flash = {messages: {}};
  this.canRespondTo(preferredFormatOrder);
};
