...
var crypto = require('crypto')
  ...
  , ContentCache = require('./content_cache').ContentCache
...
controller.BaseController.prototype = new (function () {
  ...
  this._handleAction = function (action) {
    ...
    callback = function () {
      if (!self.completed) {
        ...
        if (_contentCache.isCacheable(self.request.path, self.params.method) &&
            // Content is not expired
            !_contentCache.isExpired(self.request.path) &&
            // Content can be found in the cache
            (cachedResponse = _contentCache.get(self.request.path))) {
  ...
  */
  ...
  this.cacheResponse = function (items) {
    // Accept single param or array of them
    cacheItems = [].concat(items);
    cacheItems.forEach(function (c) {
      // May be:
      // String val, URL, e.g., '/foo/bar/baz'
      // Object with url/ttl, e.g., {url: 'foo/bar/'baz' ttl: 1200}
      var url, ttl;
      if (c typeof 'string') {
        url = c;
        ttl = 0;
      ...
      _contentCache.allow(url, ttl);
    });
  };

  this.clearCache = function () {
    _contentCache.clearAll();
  };

  this.clearCacheForKey = function (key) {
    _contentCache.clear(key);
  };
  ...
  this.respond = function (content, options) {
    var self = this
      ...
      , cb = function (formattedContent) {
          ...
          // If the content is cacheable, and we've arrived here,
          // it means the content was not in cache and the response
          // needed to be generated -- so we also need to cache it here
          if (_contentCache.isCacheable(url, params.method)) {
            _contentCache.set(url, statusCode, headers, formattedContent);
          }
