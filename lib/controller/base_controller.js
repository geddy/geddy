...
controller.BaseController.prototype = new (function () {
  ...
  */
  ...
  _execFilters = function (action, phase, callback) {
    var self = this
      , connectCompat = nails.config.connectCompatibility
  ...
  _negotiateContent = function (frmt) {
    ...
    if (types.length) {
      ...
      // Otherwise look through acceptable formats and see if Nails knows about them
      else {
        for (var i = 0, ii = types.length; i < ii; i++) {
          ...
          if (match) {
            ...
            // Nails doesn't know about this format
            _throwUndefinedFormatError.call(this);
            return;
          }
        }
  ...
  _generateSameOriginToken = function () {
    ...
    sha.update(nails.config.secret);
  ...
  */
  ...
  this._handleAction = function (action) {
    ...
    if (nails.config.secret && this.session) {
  ...
  */
  ...
  this.protectFromForgery = function (options) {
    ...
    if (!nails.config.secret) {
      nails.log.error('protectFromForgery requires an app-secret. ' +
          'Run `nails secret` in your app.');
    }
    if (!nails.config.sessions) {
      nails.log.error('protectFromForgery requires sessions.');
    }
    if (typeof opts.async != 'undefined' && !opts.async) {
      nails.log.error('protectFromForgery requires the async flag set to true.');
    }
  ...
  */
  ...
  this.renderTemplate = function (data, callback) {
    ...
    if (typeof this.layout === 'boolean' && !this.layout) {
      // Use custom Nails empty template in `lib/template/templates`
      this.layout = 'nails/empty';
    }
    else {
