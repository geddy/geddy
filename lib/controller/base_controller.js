...
controller.BaseController.prototype = new (function () {
    ...
    */
    ...
    if (typeof target == 'string') {
      ...
      if (this.name && !target.controller) {
        // kieran's router is case sensitive *waves fist*
        target.controller = this.name.toLowerCase();
      }
  ...
  */
  ...
  /*
    @name controller.BaseController#respond_with
    @public
    @function
    @description Performs content-negotiation, and renders a response.
    @param {Object} content The object to respond with.

  /*
    @name controller.BaseController#respond_with
    @public
    @function
    @description Sets a flash, then renders a response or performs a redirect.
    @param {Object|Array} content The model or collection of models to respond with.
    @param {Object} [options] Options.
      @param {String|Object} [options.status] The desired flash message, can be a string or an errors hash
  ...
  */
  this.respondWith = function (content, options) {
    ...
    // Determine the type of model from content
    var type;
    ...
    var params = this.params
      ...
      , successRedirects = {
          // FIXME: Perhaps create/update should redirect to {id: content.id} ?
          create: {}
        , update: {}
        , destroy: {}
        }
      , failureRedirects = {
        ...
        * @return {Boolean|Object} - false if no redirect should occur, a route object if otherwise
        */
    ...
    if(typeof content === 'object' && content.type && content.toObj) {
      ...
      if(redirect) {
        ...
        // Respond in the style {model: {attr: val, attr2, val...}, params: {}}
        response = {params: params};
        response[type] = content.toObj();
        response[type].id = content.id;

        this.respond.call(this, response);
      }
    }
    // Determine if `content` is a collection of models
    else if(content instanceof Array) {
      if(!type) {
        throw new Error('Cannot determine model type from empty array, specify one in opts');
      }
      ...
      throw new Error('respondWith expects either a model or collection of models');
    }
  };
