...
Cookie.prototype = new (function () {
  ...
  this.read = function (session, callback) {
    var controller = session.controller
      , secret = nails.config.secret
      , crypted = controller.cookies.get(nails.config.cookieSessionKey)
      , decipher
      , data = {};

    if (!secret) {
      throw new Error ('No app-secret found. Must run "nails secret".');
    }
  ...
  this.write = function (session, callback) {
    ...
    controller.cookies.set(nails.config.cookieSessionKey, crypted, opts);
    callback();
  };
