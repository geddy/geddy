...
Redis.prototype = new (function () {
  ...
  this.setup = function (callback) {
    var port = nails.config.sessions.server.port || null;
    var host = nails.config.sessions.server.host  || null;
    var opts = nails.config.sessions.server.opts  || {};
    _ns = nails.config.sessions.server.ns || "";
    _client = RedisServer.createClient(port, host, opts);
    _client.on("error", function (err) {
      utils.log.error("Redis error " + _client.host + ":" + _client.port + " - " + err);
    });

    if (nails.config.sessions.server.auth) {
      _client.auth(nails.config.sessions.server.auth);
    }
  ...
  this.write = function (session, callback) {
    ...
    _client.hmset(id, self._serialize(data), function (err) {
      ...
      if (nails.config.sessions.expiry) {
        _client.expire(id, nails.config.sessions.expiry)
      }
      callback();
    });
  };
