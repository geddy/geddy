...
Couchdb.prototype = new function () {
  ...
  this.setup = function (callback) {
    ...
    this.request({url: '/' + nails.config.sessions.dbName,
        method: 'GET'}, this.ensureSetup);
  };

  this.ensureSetup = function (response) {
    if (response.statusCode == 404) {
      _this.request({url: '/' + nails.config.sessions.dbName,
  ...
  this.request = function (params, callback) {
    ...
    var headers = {host: nails.config.sessions.hostname};
    if (req.data) {
      headers['content-length'] = req.data.length;
    }

    var client = http.createClient(nails.config.sessions.dbPort, nails.config.sessions.hostname);
