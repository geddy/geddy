...
nails.inFlight = new InFlight();
nails.inflection = utils.inflection;
nails.model = model;
nails.utils = utils;
nails.errors = errors;
nails.viewHelpers = {};

// Use Nails logger as the utility logger
utils.log.registerLogger(nails.log);
...
var App = function () {
  ...
  var _getDirList = function (dirname) {
        ...
        for (var i = 0; i < dirList.length; i++) {
          ...
          if (JSPAT.test(fileName)) {
            ...
            ctorName = nails.string.camelize(fileName, {initialCap: true});
  ...
  // ==================
    , _loadSessionStore = function (next) {
        sessionsConfig = nails.config.sessions;
        if (sessionsConfig) {
  ...
  // ==================
    , _loadMetrics = function (next) {
        if (nails.config.metrics) {
          nails.metrics = require('./metric');
        }
        next();
      }
  ...
  // ==================
    , _registerTemplatePaths = function (next) {
        var viewsPath = path.normalize('app/views')
          , nailsTemplatesPath = path.join(__dirname, 'template', 'templates');
        ...
        if(!utils.file.existsSync(viewsPath)) {
          ...
          var files = utils.file.readdirR(viewsPath)
            , nailsTemplatesFiles = utils.file.readdirR(nailsTemplatesPath)
          ...
          createTemplates = function (dir, isNails) {
          ...
          createTemplates(nailsTemplatesFiles, true);
  ...
  // ================================================
    , _afterStart = function (next) {
        nails.server.listeners('request');
        ...
        if (nails.config.realtime) {
          nails.io = utils.file.requireLocal('socket.io').listen(nails.server, {'log level': 0});
          ...
          , registerEventListener = function (model, event) {
            nails.model[model].on(event, function (data) {
              var id;
              ...
              nails.io.sockets.emit(model+':'+event, data);
              nails.io.sockets.emit(model+':'+event+':'+id, data);
            });
          }

          for (var i in nails.model.descriptionRegistry) {
            for (var e in events) {
  ...
  this.start = function (callback) {
    var self = this
      ...
      , router = this.router
      , allRequestTimer = nails.config.metrics && nails.metrics.Timer('Nails.all-requests')
      , controllerActionTimers = {};

    // Handle the requests
    // ==================
    nails.server.addListener('request', function (req, resp) {
      var reqUrl
        , reqObj
        , respObj
        , params
      ...
      if (router) {
        ...
        inFlightId = nails.inFlight.addEntry({
          request: req
        , method: method
        , response: respObj
        , accessTime: accessTime
        });
        reqObj._nailsId = inFlightId;
        respObj._nailsId = inFlightId;
        ...
        respObj.resp.addListener('finish', function () {
          var id = respObj._nailsId
            , entry = nails.inFlight.getEntry(id)
            , req = entry.request
          ...
          try {
            nails.log[level](logMsg);
          }
          catch(e) {
            nails.log.error('Logging failed for request, id ' + id);
          }

          nails.inFlight.removeEntry(id);
          ...
          if (nails.config.metrics) {
            ...
            controllerActionTimers[controllerName][actionName] =
              controllerActionTimers[controllerName][actionName] ||
              nails.metrics.Timer("Nails."+controllerName + '.' + actionName);
      ...
      if (params) {
          ...
          if (ctor) {
            ...
            if (typeof controller[params.action] == 'function') {
              ...
              // in-flight data
              nails.inFlight.updateEntry(reqObj._nailsId, {
                controller: controller
              });

              controller.cookies = new CookieCollection(req);

              if (nails.config.sessions) {
                controller.session =
      ...
      // Either static, 404, or 405
        ...
        staticPath = nails.config.staticFilePath + decodeURIComponent(reqUrl);
        // Ignore querystring
