...
var App = function () {
  ...
  var _getDirList = function (dirname, subdir) {
        ...
        for (var i = 0; i < dirList.length; i++) {
          ...
          // added support for subdirectories @h2non 07/09/2012 
          if (fs.lstatSync(dirname + '/' + dirList[i]).isDirectory()) {
              subret = _getDirList(dirname + '/' + dirList[i], true);
              if (subret.length >= 1) {
                  for (var x = 0; x < subret.length; x++) {
                      ret.push(subret[x]);
                  }
              }
              continue;
          ...
          if (JSPAT.test(fileName)) {
            ...
            // subdir custom names
            if (subdir === true) {
                subdirpath = dirname.split('/');
                subdirpath = subdirpath[subdirpath.length - 1];
                ctorName = geddy.string.camelize(subdirpath, {initialCap: true}) + '/' + ctorName;
            }
            ret.push({
                ctorName: ctorName
              , filePath: filePath
  ...
  // ==================
    , _registerControllers = function (next) {
        ...
        for (var i = 0; i < dirList.length; i++) {
          item = dirList[i];
          if (item.ctorName.indexOf('/')) {
              ctorFinalName = item.ctorName.split('/');
              ctorFinalName = ctorFinalName[ctorFinalName.length - 1];
              ctor = require(item.filePath)[ctorFinalName];
          } else { 
          ...
          try { 
            ...
            this.controllerRegistry[item.ctorName] = ctor;
              ...
              console.error('Error: controller ('+item.ctorName+') class is not defined. ' + e);
              //throw new Error('Error: controller class is not defined. ' + e);
          }
