...
Metric.prototype.newMetric = function (type, eventType) {
  if (nails.config.metrics.namespace) {
    eventType = nails.config.metrics.namespace + "." + eventType;
  }
...
Metric.prototype.forwardMessage = function (method, args) {
  this.messagePasser.sendMessage({
    ...
    , metricMethod: method
    , metricArgs: args
    , eventType: (nails.config.metrics.namespace) ?
                   nails.config.metrics.namespace + "." + this.eventType
                 : this.eventType
  });
}
...
module.exports.Counter = function (eventName) {
  return new Metric(nails.worker, eventName, 'counter');
}
module.exports.Meter = function (eventName) {
  return new Metric(nails.worker, eventName, 'meter');
}
module.exports.Histogram = function (eventName) {
  return new Metric(nails.worker, eventName, 'histogram');
}
module.exports.Timer = function (eventName) {
  return new Metric(nails.worker, eventName, 'timer');
}
