...
var App = function () {
  ...
  // ==================
    , _registerTemplatePaths = function (next) {
        ...
        if(!utils.file.existsSync(viewsPath)) {
          ...
          if((geddy.config.includePaths) && (geddy.config.includePaths.templates)) {
		  	for(var i = 0; i < geddy.config.includePaths.templates.length; i++) {
			  var additionalTemplateFiles = utils.file.readdirR(geddy.config.includePaths.templates[ i ]);
			  createTemplates(additionalTemplateFiles);
			}
		  }
  ...
  this.reloadTemplatePaths = function (next)
  {
    return _registerTemplatePaths.call( this, next );
  };
