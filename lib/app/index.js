  ...
  // ==================
    , _registerTemplatePaths = function (next) {
        ...
        if(!utils.file.existsSync(viewsPath)) {
          ...
          if(geddy.config.additionalTemplatePaths) {
		  	for(var i = 0; i < geddy.config.additionalTemplatePaths.length; i++) {
			  var additionalTemplateFiles = utils.file.readdirR(geddy.config.additionalTemplatePaths[ i ]);
