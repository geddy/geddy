...
, handleForwardedIp: function (reqObj) {
    if (geddy.config.reverseProxyHeader) {
      if (reqObj.headers[geddy.config.reverseProxyHeader.toLowerCase()]) {
        // reqObj.connection.remoteAddress = reqObj.headers[geddy.config.reverseProxyHeader.toLowerCase()].split(/, /)[0];
        reqObj.socket._peername.address = reqObj.headers[geddy.config.reverseProxyHeader.toLowerCase()].split(/, /)[0];
      }
    }
  }
};
