...
, enhanceRequest: function (req) {
    ...
    requestHelpers.handleForwardedIp(reqObj);
...
, handleForwardedIp: function (reqObj) {
    if (geddy.config.reverseProxyHeader) {
      if (reqObj.headers[geddy.config.reverseProxyHeader.toLowerCase()]) {
        var realIp = reqObj.headers[geddy.config.reverseProxyHeader.toLowerCase()].split(/, /)[0];

        if (reqObj.socket._peername) {
          reqObj.socket._peername.address = realIp;
        }
        else {
          reqObj.socket._getpeername = function() { return { address: realIp } };
        }
      }
    }
  }
