    ...
    var _readConfig = function (next) {
      ...
      , _initLogging = function (next) {
          ...
          types.forEach(function (type) {
            ...
            // Write to file
            if (writing) {
              currentLog = path.join(dir, type + '.log');
    ...
    this.createWorkers = function () {
      ...
      if (needed) {
        ...
        while (currentCount < configCount) {
          currentCount++;
          this.createWorker(retirement, currentCount);
          retirement -= staggerInterval;
        }
      }
    };

    this.createWorker = function (dt, workerNumber) {
      var self = this
        ...
        , workerConfig = utils.enhance(this.config, { workerNumber: workerNumber });
