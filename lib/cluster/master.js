...
Master.prototype = new (function () {
  ...
  this.init = function () {
    ...
    this.opts = null;
    this.config = {};
    this.workers = new nails.SortedCollection();
  ...
  this.start = function (options) {
    ...
    if (this.opts.nailsRoot) {
      gr = utils.file.absolutize(this.opts.nailsRoot);
      if (!fs.existsSync(gr)) {
        throw new Error('nails-root directory ' + gr + ' does not exist.');
      }
      process.chdir(gr);
    }
    ...
    utils.file.searchParentPath('config', function (err, filePath) {
      if (err) {
        console.log('Nails needs an app to run. See "nails -h" for help.');
        process.exit();
      }
      ...
      chain = new nails.async.SimpleAsyncChain(items, self);
      chain.last = function () {
